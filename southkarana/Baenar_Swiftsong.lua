...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 20373})) then
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20371})) then
		...
		e.other:QuestReward(e.self,0,0,0,0,20368); -- Item: Note to Maligar
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20367})) then
		...
		e.other:QuestReward(e.self,0,0,0,0,20366); -- Item: Mahlins Mystical Bongos
	end
