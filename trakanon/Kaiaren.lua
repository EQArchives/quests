...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("GAAAHH!! Go away! Goooowwwaaayyy!!!  Get it away from me!!");
...
function event_trade(e)
	...
	local text = "Please hand me the demon's fangs as well as the Celestial Fists book so that it may never fall into the wrong hands again.";

	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 1683},0)) then --Celestial Fists
		e.self:Emote("looks up at you wide-eyed and suddenly attacks!");
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 1688, item2 = 1689},1,text)) then --Demon Fangs and Book of Celestial Fists
